...
/**
 * Create traces based on unique label in arr_label.
 * @param arr_x
 * @param arr_y
 * @param arr_z
 * @param arr_label
 * @param arr_ids
 * @returns Array contains {x: arr_cx, y:arr_cy, z: arr_cz, label: label, arr_id:arr_cid}
 */
function get_3d_cluster_data(arr_x, arr_y, arr_z, arr_label, arr_ids){
	...
	for(var idx in arr_unique_label){
		...
		// Get index of current label and use the indexes to find x,y,z
		...
		for(var xi in idx_cluster_data){
			...
			arr_cy.push(pt_y);
			arr_cz.push(pt_z);
			if (arr_ids != undefined && arr_ids.length > 0){
				var pt_id = arr_ids[idx_cluster_data[xi]];
				arr_cid.push(pt_id);
			}	
		}
		cluster_data.push({x: arr_cx, y:arr_cy, z: arr_cz, label: label, arr_id:arr_cid});
	}
...
/**
 * get trace object from provided data
 * @param arr_x
 * @param arr_y
 * @param arr_id
 * @param arr_text - text when hover no dot
 * @param mode - combination of marker, text
 * @param name - 
 * @param legendgroup - text legend
 * @param marker - marker
 * @returns
 */
function get_3d_trace_scatter(arr_x, arr_y, arr_z, arr_id, arr_text, mode, name, legendgroup, marker){
	var trace;
	...
	trace['mode'] = custom_mode;
	trace['type'] = 'scatter';
	trace['text'] = arr_text;
	trace['textposition'] =  'top center';
	trace['textfont'] = {family:'Times New Roman'};
	trace['marker'] =  marker;
