...
$(document).ready(function() {
	...
	$('#form_upload_file').on('submit', function(e){
		...
		var output_file = document.getElementById('output_file').files[0];
		formData.append('output_file', output_file);
		...
		$.ajax({
			...
			success : function(resp) {
				...
				var n_disp_features = 0;
				if (resp.plot_feature_ranking.importances.length > 10){
					n_disp_features = 10;
				}else{
					n_disp_features = resp.plot_feature_ranking.importances.length;
				}
				var arr_fx_original = resp.plot_feature_ranking.importances.slice(0, n_disp_features);
				
				// round numbers for feature importance
				arr_fx = arr_fx_original.map(function(each_element){
