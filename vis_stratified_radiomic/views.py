...
def process_data_handler(request):
    ...
    if form.is_valid():
        ...
        new_data_file = form.cleaned_data["new_data_file"]
        ...
        if data_file:
            ...
            df_X_ori2d = pd.DataFrame(data=X_ori2d, columns=['x', 'y'])
        ...
        df_add_data = pd.DataFrame()
        df_data_id = pd.DataFrame()  # For unique ID to add to data point
        if add_data_file:
            ...
            df_data_id = df_add_data.iloc[:, 0]
       
        # Join base space X, y ==> y, x, y
        df_original = df_y_ori.join(df_X_ori2d)
        
        # Optional: Add unique key to data point
        if not df_data_id.empty:
            # Join id at the first column to point_id, x, y data
            df_data_id = pd.DataFrame(data=df_data_id.values, columns=['point_id'])
            df_original = df_data_id.join(df_original)
         
        plot["original_data"] = df_original.to_json()
        ...
        if not df_add_data.empty:
            data_table['table_columns'] = df_add_data.columns.tolist()
            data_table['table_data'] = df_add_data.to_json()
        ...
        if new_data_file:
            ...
            if label_column_header == "on":
                label_column_header_idx = 0
            df_new_data = DataFrameUtil.file_to_dataframe(new_data_file, header=new_column_header_idx)   
            # Process data with pipeline of selected algorithm
            X_new_scaled, y_predict = predict_new_data(df_new_data)
            df_predicted_label = pd.DataFrame(data=y_predict, columns=['label'])
            X_new2d = PcaUtil.reduce_dimension(X_new_scaled, n_components=2)
            df_X_new2d = pd.DataFrame(data=X_new2d, columns=['x', 'y'])
            df_new = df_predicted_label.join(df_X_new2d)
            # If additional info for predict data is uploaded, get ID from the file
            plot['new_data'] = df_new.to_json()
        ...
        if general_data_file:
            ...
            df_general_info = DataFrameUtil.file_to_dataframe(general_data_file, header=general_data_column_header_idx)
            # Optional: Add unique key to data point
            # Join id at the first column to point_id, x, y data
            df_data_new_id = pd.DataFrame(data=df_general_info.iloc[:, 0].values, columns=['point_id'])
            df_new = df_data_new_id.join(df_new)
            plot['new_data'] = df_new.to_json()
        ...
        if not df_general_info.empty:
            data_table['general_data'] = df_general_info.columns.tolist()
            data_table['general_label'] = df_general_info.to_json()
        
        # Prepare data for visualize
        resp_data['plot'] = plot
        resp_data['data'] = data_table
    else:
